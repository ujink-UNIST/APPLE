...
class AppleJobStatusResponse(BaseModel):
    ...
    error_kind: str | None = Field(None, description="첫 번째(근본) 오류 종류. 성공/대기/실행 중이면 null")
    errors: list[str] = Field(default_factory=list, description="발생 순서대로 정렬된 오류 코드 목록")
    schema_version: str = Field(SCHEMA_VERSION, description="응답 스키마 버전")
    elapsed_sec: float | None = Field(None, ge=0, description="실행 소요 시간(초). 완료 전이면 null")
    results_csv: str | None = Field(None, description="results.csv 내용. 완료 전/실패/파일 없음이면 null")
